...
router.post('/magic-link', async (req, res) => {
  try {
    ...
    if (!user) user = await User.create({ email, provider: 'local' });
    user.magicLinkToken = token;
    user.magicLinkExpires = Date.now() + 15 * 60 * 1000;
    await user.save();
    // Configure nodemailer for real SMTP in production
    const transporter = nodemailer.createTransport({
      host: 'smtp.example.com', port: 587, secure: false, auth: { user: 'user', pass: 'pass' },
    });
    await transporter.sendMail({
      from: 'no-reply@igotthis.com', to: email, subject: 'Your Magic Login Link', text: `Click to login: ${magicLink}`,
    });
